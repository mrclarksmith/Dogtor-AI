...
from gradient_accumulate import GradientAccumulator
...
class BasedTrainer(metaclass=abc.ABCMeta):
    ...
    def __init__(self, steps, epochs ):
        ...
        self.writer = tf.summary.create_file_writer("./out/")
    ...
    def run(self):
        """Run training."""
        self.tqdm = tqdm(
            initial=self.steps, total=4000000, desc="[train]"
        )
        while True:
            self._train_epoch()
...
class GanBasedTrainer(BasedTrainer):
    ...
    def __init__(
        ...
        """
        super().__init__(steps, epochs)
        self._is_generator_mixed_precision = False  
        self._is_discriminator_mixed_precision = False
    ...
    def _check_train_finish(self):
        ...
        if self.steps >= 4000000:
            self.finish_train = True

        if (
            self.steps != 0
            and self.steps == 100000
        ):
            self.finish_train = True
            logging.info(
...
class Seq2SeqBasedTrainer(BasedTrainer, metaclass=abc.ABCMeta):
    ...
    def __init__(
        self, steps, epochs, strategy, is_mixed_precision=False,
    ):
        """Initialize trainer.

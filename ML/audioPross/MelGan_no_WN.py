...
class ReflectionPad(keras.layers.Layer):
    def __init__(self, padding, **kwargs):
        ...
        super().__init__(name="reflection_pad")
...
def residual_stack(input, filters, dilation):
    ...
    x = layers.Conv1D(filters, kernel_size=3, dilation_rate=dilation, padding="same")(x)
    x = layers.LeakyReLU(0.2)(x)
    x = layers.Conv1D(filters, kernel_size=1, dilation_rate=1, padding="same")(x)

    shotcut = layers.Conv1D(filters, kernel_size=1, dilation_rate=1)(input)
...
def create_generator(input_shape, n_residual_layers=3):
    ...
    x = layers.Conv1D(512, 7)(x)

    for i, r in enumerate(ratios):
        x = layers.LeakyReLU(0.2)(x)
        x = layers.Conv1DTranspose(conv_dim, kernel_size=r*2, strides=r, padding="same")(x)
    ...
    x = layers.LeakyReLU(0.2)(x)
    # x = ReflectionPad(3)(x)
    x = layers.Conv1D(1, 7, activation="tanh", name='conv1D_generator', padding="same")(x)
    return keras.Model(inp, x)


def discriminator_block(input, layer_name):
    conv1 = layers.Conv1D(16, 15, 1, "same")(input)
    lrelu1 = layers.LeakyReLU(0.2)(conv1)
    drop1 = layers.Dropout(rate=DROP_OUT)(lrelu1)
    conv2 = layers.Conv1D(64, 41, 4, "same", groups=4)(drop1)
    lrelu2 = layers.LeakyReLU(0.2)(conv2)
    conv3 = layers.Conv1D(256, 41, 4, "same", groups=16)(lrelu2)
    lrelu3 = layers.LeakyReLU(0.2)(conv3)
    conv4 = layers.Conv1D(1024, 41, 4, "same", groups=64)(lrelu3)
    lrelu4 = layers.LeakyReLU(0.2)(conv4)
    # drop2 = layers.Dropout(rate=0.2)(lrelu4)
    conv5 = layers.Conv1D(1024, 41, 4, "same", groups=256)(lrelu4)

    lrelu5 = layers.LeakyReLU(0.2)(conv5)
    conv6 = layers.Conv1D(1024, 5, 1, "same")(lrelu5)
    lrelu6 = layers.LeakyReLU(0.2)(conv6)
    drop3 = layers.Dropout(DROP_OUT)(lrelu6)
    conv7 = layers.Conv1D(1, 3, 1, "same", name=f'conv1d_final_disc_{layer_name}')(drop3)
    return [lrelu1, lrelu2, lrelu3, lrelu4, lrelu5, lrelu6, conv7]


def create_discriminator(input_shape):
    inp = keras.Input(input_shape)
    out_map1 = discriminator_block(inp, "map1")
    pool1 = layers.AveragePooling1D(4, strides=2, padding="same")(inp)
    out_map2 = discriminator_block(pool1, "map2")
    pool2 = layers.AveragePooling1D(4, strides=2, padding="same")(pool1)
    #drop =  layers.Dropout(rate=DROP_OUT)(pool2)
    out_map3 = discriminator_block(pool2, "map3")
    return keras.Model(inp, [out_map1, out_map2, out_map3])


def init_generator_discriminator():
...
class MelGAN(keras.Model):
    ...
    def restore(self, checkpoint):
        checkpoint.restore(tf.train.latest_checkpoint('./training_checkpoints_NWN'))
        print("restored")
...
def generate_samples_audio(generator, number_of_audio_samples, save=True, lite=False):
    # code to load model
    ...
    if lite:
        audio_generated = run_lite_model(mel_set, generator)
    else:
...
def Mel_gan_load_model_from_checkpoint():
    ...
    gen_optimizer, disc_optimizer = get_optimizer(
        opt_type="adam", lr_g=LEARNING_RATE_GEN, lr_d=LEARNING_RATE_DISC)
...
def save_model(save_folder=".", f="tf"):
    _create_folder_if_it_doesnt_exist(save_folder)
    _save_model(save_folder, f=f)


def _save_model(save_folder=".", f="tf"):
    save_path = os.path.join(save_folder, f"full_model.{f}")
    generator.save(save_path, save_format=f)
...
def load_model_tf(save_folder, f="tf"):
    print(save_folder)
    save_path = os.path.join(save_folder, f"full_model.{f}")
    print(save_path)
    return tf.keras.models.load_model(save_path,
                                      custom_objects={'ReflectionPad': ReflectionPad
                                                      })


def save_lite_from_keras(model, directory):
    import pathlib
...
# def main_1():
    ...
    # save_model("mel_gan_save", f="h5")

    # generator_loaded = load_model_tf("mel_gan_save", f="h5")
...
if __name__ == "__main__":
    ...
    # X_train, y_train = gen_data(wavs[:3150])

    # generator, discriminator = init_generator_discriminator()
    # mel_gan = MelGAN(generator, discriminator)
    # gen_optimizer, disc_optimizer = get_optimizer(
    #     opt_type="adam", lr_g=LEARNING_RATE_GEN, lr_d=LEARNING_RATE_DISC)
    # checkpoint, checkpoint_prefix = init_checkpoint(gen_optimizer, disc_optimizer)
    ...
    # generator_loaded = load_model_tf("mel_gan_save", f="h5")
    # mel_gan = compile_model(mel_gan, gen_optimizer, disc_optimizer)
    # mel_gan = fit(mel_gan, X_train, y_train, epochs=150, batchsize=64)
    # mel_gan.save(checkpoint, checkpoint_prefix)

    # mel_gan, generator = Mel_gan_load_model_from_checkpoint()
    # vae = VAE.load("model_no_eager_1")
    number_of_audio_samples = 1
    # generate_samples_audio(generator, number_of_audio_samples, save=False)

    # Make lite model
    # save_lite_from_keras(generator, "model")

    lite_model = load_lite("model")
    # print("laded model")
    generate_samples_audio(lite_model, number_of_audio_samples, save=True, lite=True)
